...
* 按功能分类
  ...
  * 关系运算符：`>, <, >=, <=, ==, !==, ===, !=== ` 
    ...
    * 字符串之间做比计较，不会转为数值，而是从左到右逐个比较字符的 unicode，知道条件不满足为止；
  ...
  * 三目运算符（条件运算符）：`condition_expression ? result_A : result_B`
...
* 关于 `switch` 判断语句
  ...
  * `switch` 和`case` 的条件可以是常量、变量以及表达式：
  ...
  switch (122 + 1) {case 122 + 1: console.log('condition of case is an expression')};	
